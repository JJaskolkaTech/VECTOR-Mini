...
flowchart TD
    ...
    D --> F[Simulated KINETIC assistance]
    E --> G[Virtual finger model]
    F --> G
    G --> C
```

It does **not** demonstrate nerve repair, clinical rehabilitation efficacy, safe human stimulation, or diagnosis. Every waveform is synthetic, every output is normalized, and no code path communicates with stimulation hardware.

## Precise terminology

| Layer | What it represents | What it does not represent |
| --- | --- | --- |
| Voluntary command | A simulated attempt to extend a finger | A direct brain recording |
...
| KINETIC command | A bounded request to the simulated mechanical-assistance channel | A command to physical exoskeleton hardware |
...
The impaired pattern is not patient data and is not a universal model of paralysis, stroke, spinal-cord injury, or chronic pain. Some people and conditions may provide usable residual sEMG; others may not. Real control would require individual calibration, validated acquisition hardware, signal-quality monitoring, and a fallback intent source when residual sEMG is inadequate.

VECTOR calculates a trailing RMS envelope, normalizes it against a simulated personal calibration, combines activation level with positive trend evidence, smooths the result, and requires an 80 ms qualification dwell before declaring intent. Hysteresis prevents threshold chatter.
...
| Simulated control | Nominal behavior |
| --- | --- |
...
A drop in confidence normally returns the system to `READY` and immediately de-energizes both simulated channels.
...
| Impaired-pattern envelope peak | 45% of reference |
| Unassisted peak extension | 20.15° |
| Hybrid-assisted peak extension | 53.31° |
| Simulated tracking-error reduction | 46.3% |
...
Actual human stimulation is deliberately outside this public demo. A responsible progression would be:
...
These references establish research precedent and regulatory context. They do not validate VECTOR Mini or establish that electrical stimulation replaces physical therapy, massage therapy, or medical care.
